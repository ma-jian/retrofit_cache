...
class RetrofitCache internal constructor(
    ...
    init {
        ...
        while (iterator.hasNext()) {
            ...
            if (interceptor.javaClass.isAnnotationPresent(IgnoreInterceptor::class.java)) {
                val cacheHandle = interceptor.javaClass.getAnnotation(IgnoreInterceptor::class.java)?.enableCacheHandling
                if (cacheHandle == true) {
    ...
    fun <T> create(service: Class<T>): T {
        ...
        return Proxy.newProxyInstance(service.classLoader, arrayOf<Class<*>>(service), object : InvocationHandler {
            ...
            override fun invoke(proxy: Any, method: Method, args: Array<Any>?): Any? {
                ...
                val serviceMethod = loadServiceMethod(service, method)
                val realArgs = args ?: arrayOf()
                val invokeMethod = serviceMethod::class.java.methods.first { it.name == "invoke" }
                val isSuspend = method.parameterTypes.isNotEmpty() &&
                    method.parameterTypes.last() == Continuation::class.java
                //如果是协程方法 则需要获取最后一个参数 作为 continuation 否则 创建一个默认的continuation
                val continuation = if (isSuspend) realArgs.last() else object : Continuation<Any?> {
                    override val context: CoroutineContext get() = EmptyCoroutineContext
                    override fun resumeWith(result: Result<Any?>) {}
                }
                val businessArgs = if (isSuspend) realArgs.dropLast(1).toTypedArray() else realArgs
                return invokeMethod.invoke(serviceMethod,proxy,businessArgs,continuation)
            }
        }) as T
    }
    ...
    internal fun loadRetrofit(host: HOST): Retrofit {
       return loadHostRetrofit(host)
    }
